                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {blockableChars.map((char) => (
                            <button
                                ...
                                className="py-2.5 rounded-lg font-semibold text-sm bg-purple-500/10 border border-purple-500/40 text-purple-400 hover:bg-purple-500/20 transition-all flex items-center justify-center gap-2"
                            >
                                {CHAR_IMAGE[char] ? (
                                    <img src={CHAR_IMAGE[char]} alt={CHAR_NAME[char]} className="w-5 h-5 rounded object-cover" draggable={false} />
                                ) : (
                                    <span>🛡️</span>
                                )}
                                Bloquear ({CHAR_NAME[char]})
                            </button>
                        ))}
                    </div>
                )}
