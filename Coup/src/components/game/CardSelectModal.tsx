                ...
                <div className="flex gap-3 justify-center">
                    {aliveCards.map(({ card, index }) => (
                        <button
                            ...
                            className="rounded-xl border border-red-500/30 bg-red-500/5 hover:bg-red-500/15 hover:border-red-500/50 transition-all hover:-translate-y-1 active:translate-y-0 text-center overflow-hidden w-24"
                        >
                            ...
                            <div className="text-xs text-red-300 font-medium py-1.5">{CHAR_NAME[card.character]}</div>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="flex gap-2 justify-center flex-wrap">
                {allCards.map((char, idx) => (
                    <button
                        key={idx}
                        onClick={() => toggle(idx)}
                        className={`rounded-xl border transition-all text-center overflow-hidden w-24 ${selected.includes(idx)
                            ? 'border-emerald-500/60 bg-emerald-500/15 scale-105 ring-2 ring-emerald-500/40'
                    ...
                    >
                        ...
                        <div className={`text-[10px] font-medium py-1.5 ${selected.includes(idx) ? 'text-emerald-300' : 'text-gray-400'}`}>
                            {CHAR_NAME[char]}
                        </div>
                    </button>
                ))}
